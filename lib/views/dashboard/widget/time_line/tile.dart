...
import 'package:main/views/dashboard/widget/time_line/axis.dart';
...
class TimelineTile extends StatelessWidget {
  const TimelineTile({
    ...
    LineStyle? afterLineStyle, this.startIcon, this.endIcon,
  })  : afterLineStyle = afterLineStyle ?? beforeLineStyle,
        assert(alignment != TimelineAlign.center || startChild == null,
        ...
        assert(alignment != TimelineAlign.manual || (lineXY != null && lineXY >= 0.0 && lineXY <= 1.0),
      ...
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final children = <Widget>[
          if (startCrossAxisSpace > 0)
            SizedBox(
              height: axis == TimelineAxis.vertical ? null : startCrossAxisSpace,
  ...
  double _indicatorTotalSize() {
    if (axis == TimelineAxis.vertical) {
      return indicatorStyle.padding.left + indicatorStyle.padding.right + (hasIndicator
              ? indicatorStyle.width
              : max(beforeLineStyle.thickness, afterLineStyle.thickness));
    }

    return indicatorStyle.padding.top + indicatorStyle.padding.bottom + (hasIndicator
...
class _TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    */
    print('index=$index isLast=$isLast');
    if (!isLast) {
      _drawAfterLine(canvas, centerAxis, position);
    }
    // if (!isFirst) {
  ...
  void _drawSingleLine(
    ...
    print('_drawSingleLine()');
    //if (!isFirst) {
    final beginTopLine = axis == TimelineAxis.vertical
        ? Offset(
      centerAxis,
      position.objectSpace.start,
    )
        : Offset(
      position.objectSpace.start,
      centerAxis,
  ...
  void _drawAfterLine(Canvas canvas, double centerAxis, AxisPosition position) {
    print('_drawAfterLine()');
    final beginBottomLine = axis == TimelineAxis.vertical
        ? Offset(centerAxis, position.firstSpace.end * 0.1)
        : Offset(position.firstSpace.end* 0.1, centerAxis);
    final endBottomLine = axis == TimelineAxis.vertical
        ? Offset(centerAxis, position.secondSpace.end * 1.1)
        : Offset(position.secondSpace.end * 1.1, centerAxis);
